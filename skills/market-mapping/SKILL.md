...
5. **Estimate TAM sizing** for each segment. Ask the user for their average contract value (ACV). Then calculate:
	- **TAM** = Total companies found x ACV (total addressable opportunity if every company bought)
	- **SAM** = Companies matching the user's ICP filters x ACV (serviceable addressable market)
	- **SOM** = Companies in the user's target segment with high competitive accessibility x ACV (realistically targetable)
